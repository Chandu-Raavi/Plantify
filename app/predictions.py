...
def predict_herbal():
    ...
    try:
        ...
        max_accuracy = round(float(np.max(prediction)) * 100, 2)
        ...
        return render_template('herbal_result.html', image_filename=uploaded_file.filename,predicted_label=predicted_label,prediction_prob=max_accuracy)
    finally:
        os.remove(temp_path)